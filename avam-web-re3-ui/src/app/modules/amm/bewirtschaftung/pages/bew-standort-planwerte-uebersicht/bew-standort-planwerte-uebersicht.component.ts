...
@Component({
    selector: 'avam-bew-standort-planwerte-uebersicht',
    templateUrl: './bew-standort-planwerte-uebersicht.component.html',
    styleUrls: ['./bew-standort-planwerte-uebersicht.component.scss']
})
export class BewStandortPlanwerteUebersichtComponent implements OnInit, OnDestroy {
    @ViewChild('modalPrint') modalPrint: ElementRef;
    @ViewChild('panelTemplate') panelTemplate: TemplateRef<any>;

    dataSource: PlanwertDTO[];
    channel = 'standort-planwerte-uebersicht';
    dfeId: number;
    ...
    getToolboxConfigData(): DokumentVorlageToolboxData {
        return {
            ...
            uiSuffix: AmmPlanwerttypEnum.AMM_PLANWERTTYP_STANDORT
        };
    }
    ...
    getData() {
        ...
        forkJoin(
            ...
            ([planwerteList, standortDto, massnahme, buttons]) => {
                this.standortDto = standortDto.data;
                this.dataSource = planwerteList.data;
                this.getKurzel(massnahme.data);
                this.updateInfopanel(standortDto.data);
    ...
    updateInfopanel(data: StandortDTO) {
        this.infopanelService.dispatchInformation({
            title: 'amm.planung.label.standort',
        ...
        this.anbieter = data.durchfuehrungsortObject.unternehmenObject;
        this.infopanelService.sendTemplateToInfobar(this.panelTemplate);
    }

    getKurzel(data: MassnahmeDTO) {
    ...
    planwertErfassen() {
        this.router.navigate([`amm/bewirtschaftung/produkt/${this.produktId}/massnahmen/massnahme/standorte/standort/planwerte/erfassen`], {
            queryParams: { massnahmeId: this.massnahmeId, dfeId: this.dfeId }
        });
    }

    itemSelected(data: PlanwertDTO) {
        this.router.navigate([`amm/bewirtschaftung/produkt/${this.produktId}/massnahmen/massnahme/standorte/standort/planwerte/planwert/bearbeiten`], {
