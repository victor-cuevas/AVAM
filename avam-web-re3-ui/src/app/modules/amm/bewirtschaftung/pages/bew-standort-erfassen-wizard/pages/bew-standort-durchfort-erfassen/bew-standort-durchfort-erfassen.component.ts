...
@Component({
    selector: 'avam-bew-standort-durchfort-erfassen',
    templateUrl: './bew-standort-durchfort-erfassen.component.html'
})
export class BewStandortDurchfortErfassenComponent implements AfterViewInit, OnDestroy {
        ...
        this.bewirtschaftungRestService.saveDfeDurchfuehrungsort(this.durchsfuerungsortFormComponent.mapToDTO(), this.wizardService.dfeId).subscribe(
            response => {
                if (response.data) {
                    ...
                    this.router.navigate([`amm/bewirtschaftung/produkt/${this.wizardService.produktId}/massnahmen/massnahme/standorte/standort/grunddaten`], {
                        queryParams: { dfeId: this.wizardService.dfeId, massnahmeId: this.wizardService.massnahmeId }
                    });
                }
