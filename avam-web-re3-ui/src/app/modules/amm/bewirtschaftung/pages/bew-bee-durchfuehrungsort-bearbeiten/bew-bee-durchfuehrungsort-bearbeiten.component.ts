...
@Component({
    selector: 'avam-bew-bee-durchfuehrungsort-bearbeiten',
    templateUrl: './bew-bee-durchfuehrungsort-bearbeiten.component.html'
})
export class BewBeeDurchfuehrungsortBearbeitenComponent implements AfterViewInit, OnDestroy, DeactivationGuarded {
    ...
    getData() {
        ...
        forkJoin([
            ...
            ([beResponse, swissResponse]) => {
                const standortDto = beResponse.data;
                let dfoDto;

                if (standortDto) {
                    this.beDto = standortDto.beschaeftigungseinheiten[0];
                    this.massnahmeDto = standortDto.massnahmeObject;
                ...
                if (this.beDto) {
                    ...
                    dfoDto = this.beDto.durchfuehrungsortObject;
                    this.initInfopanel();
                    this.updateSecondLabel(this.beDto);
                }
    ...
    save() {
        ...
        this.bewirtschaftungRestService.saveBeDurchfuehrungsort(this.durchfuehrungsortFormComponent.mapToDTO(), this.beId).subscribe(
    ...
    anbieterdatenUebernehmen() {
        ...
        this.bewirtschaftungRestService.anbieterdatenUebernehmenBe(this.beId).subscribe(
            response => {
                if (response.data) {
                    this.durchfuehrungsortData = {
        ...
        this.observeClickActionSub = this.facade.toolboxService.observeClickAction(ToolboxService.CHANNEL).subscribe((action: any) => {
            if (action.message.action === ToolboxActionEnum.PRINT) {
                ...
                this.openHistoryModal(
                    ...
                    AmmConstants.VERANTWORTLICHER_PERSON
                );
            }
        });
    }
