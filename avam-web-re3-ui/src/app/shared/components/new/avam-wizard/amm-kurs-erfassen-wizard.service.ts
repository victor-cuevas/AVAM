...
@Injectable()
export class AmmKursErfassenWizardService implements OnDestroy {
    list: any;
    currentStep: number;
    ...
    isWizardNext = false;
    channel = 'kurs-erfassen-wizard';
    produktId: number;
    massnahmeId: number;
    dfeId: number;
    erfassungsspracheIdGrunddatenState: number;
    kursTitelObject: MultilanguageTitle;
    ...
    navigateStep(stepId) {
        this.router.navigate([`amm/bewirtschaftung/produkt/${this.produktId}/massnahmen/${this.massnahmeId}/kurs/erfassen/${this.routerOptions[stepId]}`]);
    }
    ...
    resetValues() {
        ...
        this.kursTitelObject = undefined;
