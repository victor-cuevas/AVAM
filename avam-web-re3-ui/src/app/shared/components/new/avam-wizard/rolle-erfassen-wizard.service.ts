...
@Injectable()
export class RolleErfassenWizardService implements OnDestroy {
    list: any;
    currentStep: number;
    ...
    isWizardNext = false;
    channel = 'rolle-erfassen-wizard';
    displayLeaveConfirmation = false;

    rolleDTO: RolleDTO;
    berechtigungen: SysFunkRolleViewDTO[];

    private routerOptions = ['grunddaten', 'berechtigungen'];
    ...
    navigateStep(stepId) {
        this.router.navigate([`informationen/benutzerverwaltung/rolle/erfassen/${this.routerOptions[stepId]}`]);
        // implement
    }
    ...
    resetValues() {
        ...
        this.rolleDTO = undefined;
        this.berechtigungen = undefined;
    }
